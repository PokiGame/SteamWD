...
def clean_name(name):
    """Clean the item name by removing special characters, unnecessary prefixes, and replacing underscores with spaces."""
    # Remove unwanted prefixes like "Steam Workshop"
    name = re.sub(r'^Steam Workshop\s*', '', name, flags=re.IGNORECASE)
    # Replace multiple underscores with a single underscore
    name = re.sub(r'_+', '_', name)
    # Replace underscores with spaces
    name = name.replace('_', ' ')
    # Remove any non-alphanumeric characters (except for spaces) and trim extra spaces
        ...
        if len(line) > 0:
            try:
                ...
                if re.search("SubscribeCollectionItem", x.text):
                    ...
                    for wid, appid in dls:
                        # Get item name
                        name = re.search(r'<title>(.*?)</title>', x.text)
                        name = name.group(1) if name else f"item_{appid}_{wid}"
                        name = clean_name(name)
                        download.append((appid, wid, name))
                elif re.search("ShowAddToCollection", x.text):
                    # single item
                    wid, appid = re.findall(r"ShowAddToCollection[\( ']+(\d+)[ ',]+(\d+)'", x.text)[0]
                    # Get item name
